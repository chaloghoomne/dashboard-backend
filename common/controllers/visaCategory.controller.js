...
module.exports = {
	...
	async editVisaCategory(req, res) {
		try {
			...
			const visaCategory = await VisaCategory.findByIdAndUpdate(
				id,
				data,
				{
					new: true,
				...
				visaCategories = visaCategories.map((visaCategory, index) => {
					...
					if (package && package.tourTypes) {
						const tourType = package.tourTypes.find(
							(type) =>
								type._id.toString() ===
								visaCategory.tourType.toString()
						);
	...
	async getVisaCategoryById(req, res) {
		try {
			...
			if (visaCategory.expressDays) {
				...
				expressDate.setDate(
					currentDate.getDate() + visaCategory.expressDays
				);
				visaCategory._doc.expressDate = new Date(
					expressDate
				).toDateString();
			}
			...
			if (visaCategory.instantDays) {
				...
				instantDate.setDate(
					currentDate.getDate() + visaCategory.instantDays
				);
				visaCategory._doc.instantDate = new Date(
					instantDate
				).toDateString();
			}
		...
		try {
			...
			visaCategories.forEach((category) => {
				if (category.expressDays) {
					const expressDate = new Date(currentDate);
					expressDate.setDate(
						currentDate.getDate() + category.expressDays
					);
					category._doc.expressDate = new Date(
						expressDate
					).toDateString();
				}
				if (category.instantDays) {
					const instantDate = new Date(currentDate);
					instantDate.setDate(
						currentDate.getDate() + category.instantDays
					);
					category._doc.instantDate = new Date(
						instantDate
					).toDateString();
				}
			});
